...
class HomeworkList extends Component {
  ...
  renderHomeworkInfo(itemInfo) {
    const {
      id, author, classInfo, commentator, description, isExcellent, teacherInfo, time
    } = itemInfo
    return (
      <div className="homework-info">
        <div>
          ...
          <span className="homework-info__item">
            佳作&nbsp;
            <Switch
              checked={isExcellent}
              onChange={() => { this.handleSwitchExcellentState(itemInfo) }}
            />
          </span>
        </div>
      </div>
  ...
  render() {
    ...
    const fullList = list.map(id => {
      ...
      return {
        ...item, photos, comments, author, classInfo, teacherInfo, time
      }
    })
    return (
      <List
